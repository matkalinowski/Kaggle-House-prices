...
def predict_with_kfold(results_class, model, param_grid, X, y, test, folds=5, yield_progress=True,
                       values_transformation=np.expm1):
    ...
    best_result.plot_results(values_transformation=values_transformation)
    if yield_progress:
...
def predict(results_class, clf, param_grid, xtrain, ytrain, xtest, name,
            ...
            predictions_form_restoring_method=None, n_jobs=-1, cv=5, plot_values_transformation=np.log1p):
    ...
    if plot_results:
        results.plot_results(values_transformation=plot_values_transformation)
    return results
